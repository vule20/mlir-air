...
| `smolvla_fuse.py` | the ELF layout switch `SMOLVLA_FUSE_FA` and the LayerNorm switch `SMOLVLA_LN_EXT` (`1` default: C++ row kernel `layer_norm/layer_norm_rows.cc`; `0`: the air.api DSL loop). FUSE_FA: (`0` default: separate `flash_attn` ELF; `1`: FlashAttention inside `vit_ln_qkv`, slower; `layer`: experimental one-ELF layer); `SMOLVLA_OFFN_TILING` / `SMOLVLA_LNQKV_TILING` (`12,6` default) set the loop tiling of the two fused ELFs |
