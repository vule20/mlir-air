...
//
// with the last multiply-add as one accumulate, i.e. one rounding, which is what
// the vector codegen of the air.api version does. The output is not bit-identical
// to it (the fp32 sums are reduced in a different order, so a few rows round
// differently), but it is as accurate: cosine against the f32 reference is the
// same to 6 digits, and the largest difference is one bf16 ulp.
//
// Why it is a C++ kernel: the air.api version spends ~2700 cycles per row, all of
// it latency, on a strictly serial chain (reduce through an L1 scratch buffer, a
// second reduce, then the epilogue). Here 32 lanes are used, and LN_GROUP rows are
// processed together so their independent chains overlap.
//
// Tuning notes (NPU2, M=3072 rows over 8 tiles): LN_ROWS == LN_GROUP == 4 is the
// best point. Larger groups spill accumulators to the stack and get slower;
